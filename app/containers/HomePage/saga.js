...
export function* getChapterFromUrl({
  ...
  const hasFormattedJson = some(filesets, (f) => f.type === FILESET_TYPE_TEXT_JSON);
  ...
  try {
    ...
    const cText = copyrights.filter(
      (c) =>
        ...
        (c.type === FILESET_TYPE_TEXT_PLAIN || c.type === FILESET_TYPE_TEXT_FORMAT || c.type === FILESET_TYPE_TEXT_JSON),
    )[0];
    const ntText = !cText
      ? copyrights.filter(
          (c) =>
