...
const getFirstChapterReference = (
	...
	if (hasVideo) {
		...
		reference = `${firstBookId}/${firstChapter >= 0 ? firstChapter : '1'}`;
		...
		const audioBooks = bookMetaResponse.find((filesetObject) =>
			audioDrama ? filesetObject[audioDrama.id] : filesetObject[audio.id],
		);
		const textBooks = bookMetaResponse.find((filesetObject) =>
