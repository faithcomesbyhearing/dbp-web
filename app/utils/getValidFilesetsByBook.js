...
const getValidFilesetsByBook = (
	...
	filesetIdsWithBooksAllowed.forEach((item) => {
		...
		filesetWithBooksAllowedIndexed[key] = filesetWithBooksAllowedIndexed[
			key
		].concat(value.books);
	});
	...
	filesetIdsForBookMetadata.forEach(([filesetType, filesetId, filesetSize]) => {
		...
		const booksAllowed = (
			filesetWithBooksAllowedIndexed[filesetId] || []
		).reduce((acc, { book_id: bookId }) => {
			acc[bookId] = true;
			return acc;
		}, {});
	...
	return filesets.filter((fileset) => {
		...
		const filesetMetadata =
			idsForBookMetadataIndexed[fileset.id]?.[fileset.type];

		if (!filesetMetadata) {
			return false;
		}

		const isCompleteTestament =
			filesetMetadata.testament === FILESET_SIZE_COMPLETE;
		const isMatchingTestament = filesetMetadata.testament.includes(
			book.testament,
		);
		const isCovenantTestament =
			FILESET_SIZE_STORIES === filesetMetadata.testament &&
			book.testament === BOOK_COVENANT_TESTAMENT;
		const isBookAllowed = filesetMetadata.booksAllowed[book.book_id];

		return (
			(isCompleteTestament || isMatchingTestament || isCovenantTestament) &&
			isBookAllowed
		);
	});
};
