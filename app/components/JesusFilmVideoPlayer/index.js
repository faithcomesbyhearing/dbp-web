...
class JesusFilmVideoPlayer extends React.PureComponent {
  ...
  componentDidMount() {
    if (this.videoRef.current) {
      this.initializeHls();
      this.videoRef.current.addEventListener('webkitendfullscreen', this.webkitendfullscreen);
  ...
  componentWillUnmount() {
    this.cleanupHls();
    if (this.videoRef.current) {
      this.videoRef.current.removeEventListener('webkitendfullscreen', this.webkitendfullscreen);
    }
  ...
  initializeHls = () => {
    if (Hls.isSupported()) {
      ...
      this.hls.on(Hls.Events.MEDIA_ATTACHED, () => {
        this.hls.loadSource(`${this.props.hlsStream}?key=${process.env.DBP_API_KEY}&v=4`);
    ...
    if (!hasVideo || !hlsStream) {
      return (
        <div className="video-player-container jesus-film-override">
          <h2>Something went wrong. Please refresh the page or go back to try again!</h2>
        </div>
      );
    }
