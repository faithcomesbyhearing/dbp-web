...
export class VersionList extends React.PureComponent {
	get filteredVersionList() {
		...
		const getKey = (bible) => {
			if (!bible) return '';
			const abbr = bible['abbr'];
			const date = bible['date'] || '2019';
			const typeSuffix = bible['hasVideo'] ? '_video' : '_plain';
			...
			const item = {
				...
				altText:
					bible['vname'] && bible['vname'] !== bible['name']
						? bible['name']
						: '',
				types: bible['filesets']
					? bible['filesets'].reduce(
							(a, c) => ({ ...a, [c['type']]: true }),
							{},
						)
					: {},
	...
	handleVersionListClick = async (bible, audioType) => {
		...
		const { toggleTextSelection, activeTextId, activeBookId, activeChapter } =
			this.props;
		const hasVideo = !!bible['hasVideo'];

		if (bible['jesusFilm']) {
			// If there is a Jesus Film then use alternate navigation logic
			Router.push(`/jesus-film/${bible['iso']}`);
			toggleTextSelection();
		...
		if (bible['abbr'].toLowerCase() === activeTextId.toLowerCase()) {
			toggleTextSelection();
			return;
		}
		...
		if (JSON.parse(sessionStorage.getItem('bible_is_maintain_location'))) {
			...
			const [filteredMetadata, allMetadata] = await getBookMetaData({
				idsForBookMetadata: bible['filesets'].map((set) => [
					set['type'],
					set['id'],
				]),
			});
			...
			Router.push(
				`/app?bibleId=${bible['abbr']}&bookId=${bookId}&chapter=${chapterId}${query || ''}`,
				`/bible/${bible['abbr']}/${bookChapterUrl}`,
			);
		}
		...
		if (bible) {
			if (audioType) {
				if (
					...
					document.cookie = `bible_is_audio_type=${audioType};path=/bible/${bible['abbr']}`;
				}
			}
			toggleTextSelection();
		}
